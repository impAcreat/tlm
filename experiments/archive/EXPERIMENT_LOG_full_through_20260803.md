...
## 2026-07-21 Round 11 (partial): full-set injection is high-churn, net-zero — motivates selective injection

Main table still running (6 arms x 274, HF greedy, 35 steps). Endpoints complete:
  mt_bad  seen 40/140 (28.6%)  unseen 31/134 (23.1%)
  mt_good seen 76/140 (54.3%)  unseen 83/134 (61.9%)

Paired per-task analysis on the 140 SHARED seen tasks (both arms complete), which is the
correct lens (aggregate ratios of partially-run arms are invalid due to task ordering):

  mt_gmb_x (bad + whole gmb, multi-layer calibrated) vs mt_bad:
    41/140 vs 40/140  (net +1)  BUT  fixed +13 / broke -12
  mt_s1_TS (s1 + T-compiled unit e0) vs mt_s1:
    57/140 vs 56/140  (net +1)  BUT  fixed +11 / broke -10

Key finding: undirected injection is NOT inert — it flips ~25 task outcomes out of 140, but
fixes and breaks in near-equal numbers, so NET success is ~zero. The broken tasks are ones the
base skill already solved; injecting a "good-skill direction" into tasks that need no repair is
pure perturbation. This is a qualitatively different (and more informative) result than "no effect".

Implication (reframes the deployment claim):
- Non-selective full-set injection = wash (net ~0). The earlier "recovers ~75% of the text effect"
  holds ONLY on repaired subsets and must NOT be extrapolated to the full heterogeneous set.
- Value requires SELECTIVITY: inject only where there is repair headroom. Evidence already points
  this way — gmb on repaired-8 gave 3/8 (vs bad 0/8), S-unit on domain-matched T_search gave 5/16
  (vs 2/16) with zero cross-domain harm. On a repaired subset the base fails everywhere, so injection
  has only upside (no success to break) — the analytical repaired-23 table (running) is the clean lens.
- This directly motivates the activation-space routing application: route the vector to tasks/states
  that need it, skip the rest.

Next: (a) finish main table for exact full-set ±churn numbers (this IS the "why routing" evidence);
(b) analytical repaired-23 table (ext_all / ext_sum / gen_sum, apply_edit, multi-layer) for the clean
selective-injection numbers and the three claims; (c) optional selectivity control — confirm the
gmb_x-broken tasks are preserved when skipped by a router.

Evidence boundary: greedy, single seed, single model; churn counts are from complete-vs-complete
paired tasks (valid), remaining injection arms still filling in.
